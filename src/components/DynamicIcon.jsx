...
export default function DynamicIcon({ name, className = 'w-5 h-5', ...props }) {
  if (!name) return <CircleDot className={className} {...props} />;
  ...
  if (lowerName === 'leetcode') {
    return <LeetCodeIcon className={className} {...props} />;
  }

  const ComponentIcon = ICON_MAP[lowerName] || CircleDot;
  return <ComponentIcon className={className} {...props} />;
}
